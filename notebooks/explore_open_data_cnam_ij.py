...
for unit in ij_tt_age["unit"].unique():
    ...
    plt.savefig(
        DIR2IMG_CNAM
        / f"ij_cnam_tout_age_{unit.replace(' ', '_').replace('\n', '_')}.png",
        bbox_inches="tight",
    )
# %%
# Analyse par âge
...
age_classes = [
    a for a in all_ij["Tranche d'âge"].unique() if a not in ["inconnu", "TOUT_AGE"]
]
...
for type_ij in all_ij["Type IJ"].unique():
    for unit in all_ij["unit"].unique():
        ...
        sns.lineplot(
            ...
            hue="Tranche d'âge",
            marker="o",
            palette=dict(zip(age_classes_sorted, palette)),
        ...
        for age in age_classes_sorted:
            ...
            ax.text(
                ...
                last_row["value"],
                str(age),
                color=dict(zip(age_classes_sorted, palette))[age],
                fontsize=20,
                fontweight="bold",
                va="center",
                ha="left",
...
age_classes = [
    a for a in all_ij_sexe["Tranche d'âge"].unique() if a not in ["inconnu", "TOUT_AGE"]
]
age_classes_sorted = sorted(age_classes, key=age_key)
palette = sns.color_palette("viridis", n_colors=len(age_classes_sorted))
palette.reverse()  # Inverser pour que les couleurs sombres soient pour les âges élevés
...
df_sexe = all_ij_sexe[
    ...
    & (all_ij_sexe["Tranche d'âge"].isin(age_classes))
]
...
for i, age in enumerate(age_classes_sorted):
    ...
    ax.hlines(
        ...
        colors=dict(zip(age_classes_sorted, palette))[age],
        linewidth=0.7,
    )
    ax.text(
...
sex_handles = [h for h, lab in zip(handles, labels) if lab in sex_labels]
...
ij_naf_filtered = all_ij_naf[
    (all_ij_naf["Type IJ"].isin(target_type_ij)) & (all_ij_naf["Libellé"] != "Ensemble")
]
for unit in ij_naf_filtered["unit"].unique():
    for type_ij in target_type_ij:
        df_plotted = ij_naf_filtered[
            (ij_naf_filtered["unit"] == unit) & (ij_naf_filtered["Type IJ"] == type_ij)
        ]
        ...
        ax.grid(which="minor", linestyle=":", linewidth="0.7", color="gray")
        plt.legend(
            title="Libellé Naf", loc="upper center", bbox_to_anchor=(0.5, -0.1), ncol=3
        )
        plt.savefig(
            DIR2IMG_CNAM
            / f"ij_cnam_par_naf_{unit.replace(' ', '_').replace('\n', '_')}.png",
            bbox_inches="tight",
        )
        plt.show()
...
df_arret_maladie_tt_category = df_ij_tt_category[
    df_ij_tt_category["Type IJ"] == "maladie-hors-derogatoires"
]
...
growth_rate_arret_maladie_tt_category["unit"] = growth_rate_arret_maladie_tt_category[
    "unit"
].str.replace(r"\s*\(.*\)\s*", "", regex=True)
# %%
agg_dict = {
    "Taux de croissance moyen (%)": pd.NamedAgg(
        column="growth_rate", aggfunc=lambda x: x.mean() * 100
    ),
    "Période": pd.NamedAgg(
...
mean_growth_rate_post_2019 = (
    growth_rate_arret_maladie_tt_category[
        growth_rate_arret_maladie_tt_category["Année"] >= start_year_p2
    ]
    .groupby("unit")
    .agg(**agg_dict)
)
...
mean_growth_rate_post_2020 = (
    growth_rate_arret_maladie_tt_category[
        growth_rate_arret_maladie_tt_category["Année"] >= start_year_p3
    ]
    .groupby("unit")
    .agg(**agg_dict)
)
...
# pivot by units
all_growth_rates = all_growth_rates.pivot(
    index="unit", columns="Période", values="Taux de croissance moyen (%)"
)
all_growth_rates.round(2).to_csv(
    DIR2RESULTS / "mean_growth_rate_arret_maladie_tt_age.csv"
)
all_growth_rates
# %%}")
#
...
nb_beneficiaires = [
    4592431, 4700234, 4607437, 4691446,
    4582189, 4773240, 4868392, 4928043, 5035167, 5078020
]
nb_jours_arrets = [
    200567081, 202946590, 205076919, 204256953,
    211665940, 217877165, 226581529, 233718607, 239553911, 245124624
]
nb_arrets = [
    6433056, 6577668, 6445983, 6544544,
    6420422, 6729644, 6896297, 6973811, 7106484, 7180525
]
