...
for df in [nb_taux_absences, nb_total_absences]:
    ...
    df["time_regex"] = df["TIME_PERIOD"].map(
        lambda x: re.search(r"(\d{4})-Q(\d{1})", x)
    )
    df["datetime"] = pd.to_datetime(
        df["time_regex"].map(lambda x: str(x.group(1)))
        + "-"
        + df["time_regex"].map(
            lambda x: str((int(x.group(2)) - 1) * 3 + 1).zfill(2)
        )
        + "-01",
        format="%Y-%m-%d",
        errors="coerce",
...
print(
    nb_taux_absences.groupby(COLNAME_COUNTRY)["datetime"]
    .max()
...
def plot_absences_europe(df, countries_dict, language= "fr", unite="percentage"):
    ...
    if language == "fr":
        ...
        eu_label  = "EU (27)"
        df_plot.loc[df_plot[col_country_label] == "European Union - 27 countries (from 2020)", col_country_label] = eu_label
    else:
        raise ValueError("language must be 'fr' or 'en'")

    covid_year = 2020
    mean_before_covid = (
        df_plot.loc[df_plot["datetime"].dt.year < covid_year]
        .groupby(col_country_label)["OBS_VALUE"]
        .mean()
    ).reset_index()
    mean_after_covid = (
        df_plot.loc[df_plot["datetime"].dt.year > covid_year]
    ...
    plt.text(
        ...
        ha="left", fontsize=16, color="grey",
    )
    # x et y labels
    ax.set_xlabel("Année")
    ...
    if unite == "percentage":
        ...
        ax.legend(handles, labels,bbox_to_anchor=(1.05, 1), loc="upper left", ncol=1, borderaxespad=0.)
    else:
        ax.legend(bbox_to_anchor=(1.05, 1), loc="upper left", ncol=1, borderaxespad=0.)
    return ax
# %%
...
nb_total_absences_q1 = nb_total_absences[
    mask_sex
...
growth_rate_total_numbers = (
    ...
    .reset_index().rename(columns={"OBS_VALUE": COLNAME_TX_CROISSANCE})
)
...
mean_growth_rate_pre_2019 = (
    growth_rate_total_numbers[
        (growth_rate_total_numbers["year"] <= end_year_p1)
        & (growth_rate_total_numbers["year"] >= start_year_p1)
    ]
)
...
mean_growth_rate_post_2020 = (
    growth_rate_total_numbers[
        (growth_rate_total_numbers["year"]  >= start_year_p2)
        & (growth_rate_total_numbers["year"] <= end_year_p2)
    ]
)
mean_growth_rate_post_2020[COLNAME_PERIOD] = period_2
all_growth_rates = pd.concat(
    [mean_growth_rate_pre_2019, mean_growth_rate_post_2020]
).reset_index().sort_values(by=[COLNAME_COUNTRY, "quarter", "Période"])
...
growth_rates_subset_countries = all_growth_rates[
    ...
    [
        *west_europe_absences.keys(), *central_europe_absences.keys(), *east_europe_absences.keys(), *nordik_absences.keys(), LABEL_ALL_EU_LONG])
        ]
growth_rates_subset_countries.loc[growth_rates_subset_countries[COLNAME_COUNTRY] == LABEL_ALL_EU_LONG, COLNAME_COUNTRY] = LABEL_ALL_EU

order = (
    growth_rates_subset_countries[
        (growth_rates_subset_countries[COLNAME_PERIOD] == period_2)
        ].groupby(COLNAME_COUNTRY).agg(
            **{COLNAME_TX_CROISSANCE: pd.NamedAgg(COLNAME_TX_CROISSANCE,"mean")}).reset_index()
...
growth_rates_subset_countries_sorted = growth_rates_subset_countries_sorted.sort_values(
    [COLNAME_COUNTRY, "quarter", COLNAME_PERIOD]
)
...
plt.show()
# %%
mean_growth_rates = growth_rates_subset_countries_sorted.groupby([COLNAME_COUNTRY, COLNAME_PERIOD])[COLNAME_TX_CROISSANCE].agg(
    ["mean", lambda x: np.std(x)/np.sqrt(len(x))]
).unstack()

selected_countries = [
    "Austria", "Romania", "Bulgaria", "Denmark", "Spain", "Czechia", "Netherlands", "France"
]
growth_rates_list = [
    f"{country} (+{mean_growth_rates.loc[country, ('mean', period_2)]:.2f} %)"
